# Consolidated from: postcord.py, postcord_v2.py, postcord1.py
# Canonical version: postcord_v2.py (adds ECL range validation + tighter candidate filtering over postcord.py)
# Dropped: postcord1.py (earliest iteration; subset of postcord.py functionality)
# Dropped: postcord.py (superseded by postcord_v2.py; lacks ECL validation and prefilter cache integration)
...
You have TWO STRICT SUBTASKS:

- TASK 1 (TRIAGE, INTERNAL ONLY): Decide whether a contraindication statement can be reasonably represented using ONLY:
  - a single focus SNOMED CT concept, plus at most these attributes:
    1) Causative agent
    2) Severity
    3) Clinical course
- TASK 2 (OUTPUT): Based on the TASK 1 decision, either:
  - perform post-coordination slot filling, or
  - indicate that post-coordination is not appropriate.

You MUST always perform TASK 1 first, but you MUST ONLY OUTPUT the result of TASK 2 in the final JSON.

--------------------
TASK 1 (INTERNAL): TRIAGE DECISION
--------------------
Goal:
Decide whether the QUERY can be represented using:
- one focus SNOMED CT concept, and
- any subset of {causative_agent, severity, clinical_course}.

Definitions:
- POSTCOORD_OK: The meaning can be captured by focus + any subset of {causative_agent, severity, clinical_course}.
- NEEDS_CONSTRAINTS: The statement includes semantics requiring additional modeling beyond these attributes.
- UNMAPPABLE: The statement is too vague or the core clinical meaning cannot be identified.

Rules:
- The output of task 1 should be one of POSTCOORD_OK, NEEDS_CONSTRAINTS, or UNMAPPABLE.
- Use POSTCOORD_OK only when the meaning fits within the 3-attribute envelope.
- If any unsupported constraint is present, choose NEEDS_CONSTRAINTS.
- If the core clinical meaning cannot be clearly identified, choose UNMAPPABLE.

Do NOT output any separate JSON for TASK 1. Use its decision only to guide TASK 2.

--------------------
TASK 2: POST-COORDINATION DECISION AND SLOT FILLING
--------------------
Task:
1) Use the internal decision from TASK 1 to set the "decision" field:
2) Analyze the QUERY, which represents a contraindication where a patient should not receive a drug.
3) Choose the single best focus concept from FOCUS_CANDIDATES.
   - The focus represents the primary condition/diagnosis/physiological state that is the target of the post-coordination.
   - For conditions, prioritize 'Disease' or 'Disorder' type concepts.
   - For physiological states and latent conditions, prioritize 'Finding' type concepts representing the 'state' or 'dispositional' aspect.
4) For each attribute in ATTRIBUTE_TABLE, select the best VALUE from its candidate list.
...
5) Regardless of the decision of TASK 1, you MUST attempt to fill in the output JSON according to the schema below. 
If TASK 1 indicates that the statement is not suitable for post-coordination, you should still attempt to find the best possible match for each attribute.
...
Schema:
{{
  "decision": "POSTCOORD_OK" | "NEEDS_CONSTRAINTS" | "UNMAPPABLE",
