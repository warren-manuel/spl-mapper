#!/usr/bin/env python3
# Consolidated from: evaluate_agg_results.py, evaluate_agg_results_1.py,
#                    evaluate_agg_results_2.py, evaluate_agg_results_2[copy].py,
#                    evaluate_agg_results_3.py
# Canonical version: evaluate_agg_results_3.py (most complete)
# Dropped: evaluate_agg_results.py — baseline exact match only
# Dropped: evaluate_agg_results_1.py — added embedding similarity but used greedy matching
# Dropped: evaluate_agg_results_2.py — added linear_sum_assignment but lacked hierarchy scoring
# Dropped: evaluate_agg_results_2[copy].py — exact duplicate of v2
...
# No logic changes — import paths updated to src.* layout.
...
try:
    from src.retrieval.embedding_utils import load_ST_model
except Exception:
    load_ST_model = None

try:
    from src.snomed.snomed_utils import _resolve_rf2_file, get_ancestors_with_depth
...
# ---------------------------------------------------------------------------
# Shared constants (imported by scripts/run_pipeline.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Utility functions extracted from agent_runner.py
# ---------------------------------------------------------------------------
...
def validate_postcoord_with_mrcm(selected_focus_id: str, fills: Dict[str, str]) -> Tuple[bool, List[str]]:
    """Lightweight validation only — slot candidates are already range-prefiltered."""
...
def _aggregate_result_item(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status in {"POSTCOORD", "MINIMAL"}:
        ...
        for key in ("causative_agent", "severity", "clinical_course"):
            ...
            if value_id == "N/A":
                continue
            attributes[key] = {"value_id": value_id, "value_term": fill_value(key, "term")}
        return {
            "SPL_SET_ID": str(result.get("SPL_SET_ID", "")),
            "item_index": int(result.get("item_index", 0)),
            "query_text": str(result.get("query_text", "")),
...
def _aggregated_item_to_csv_row(item: Dict[str, Any]) -> Dict[str, Any]:
...
def aggregate_agent_results(run_rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for row in run_rows:
        ...
        for result in row.get("results") or []:
            grouped[spl_set_id].append(_aggregate_result_item(result))
    ...
    for spl_set_id in sorted(grouped):
        ...
        csv_rows.extend(_aggregated_item_to_csv_row(item) for item in ordered_items)
...
def evaluate_aggregated_predictions(
    ...
    *,
    ...
    resolved_st_model_id = st_model_id or os.environ.get(
        "MAPPER_MODEL_NAME", "tavakolih/all-MiniLM-L6-v2-pubmed-full"
    )
    cmd = [
...
# ---------------------------------------------------------------------------
# Evaluation internals (from evaluate_agg_results_3.py)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--discard-na-gold-expression", action="store_true")
    ...
    ap.add_argument("--snomed-rel", default="")
    ap.add_argument("--snomed-source-dir", default="snomed_us_source")
    ap.add_argument("--hierarchy-partial-score", type=float, default=0.0)
    ap.add_argument("--sibling-base", type=float, default=0.4)
    ap.add_argument("--sibling-decay", type=float, default=0.1)
    ap.add_argument("--hop-penalty", type=float, default=0.25)
...
def _cached_ancestors_with_depth(concept_id: int, rel_df_indexed: "pd.DataFrame") -> Dict[int, int]:
...
def compute_tiered_concept_metrics(
    ...
    score_mat = np.array(
        [
            [
                concept_similarity_score(p, g, rel_df_indexed, partial_floor, sibling_base, sibling_decay, hop_penalty)
                for g in gold_list
            ]
            for p in pred_list
        ],
...
def write_csv_file(path: str, rows: Iterable[Dict[str, Any]], fieldnames: List[str]) -> None:
    out = Path(path)
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
...
def build_pair_scores(
    ...
    if st_model is not None:
        gold_vectors = st_model.encode(
            gold_texts, normalize_embeddings=True, show_progress_bar=False, batch_size=st_batch_size,
        )
        pred_vectors = st_model.encode(
            pred_texts, normalize_embeddings=True, show_progress_bar=False, batch_size=st_batch_size,
        )
...
def main() -> None:
    ...
    for spl, gold_rows_for_matching in gold_by_spl.items():
        ...
        if gold_rows_for_matching and pred_rows_for_matching:
            if semantic_enabled:
                score_mat, sem_mat, jac_mat = build_pair_scores(
                    gold_rows_for_matching, pred_rows_for_matching, st_model, args.alpha, args.beta, args.st_batch_size,
                )
            else:
                ...
                for gold_idx, gold_row in enumerate(gold_rows_for_matching):
                    ...
                    for pred_idx, pred_row in enumerate(pred_rows_for_matching):
                        ...
                        j = float(jaccard(gold_tokens, pred_tokens))
                        jac_mat[gold_idx, pred_idx] = j
                        score_mat[gold_idx, pred_idx] = j
        ...
        for gold_idx, gold_row in enumerate(gold_rows_for_matching):
            ...
            if gold_idx in mapping:
                ...
                if ignored_gold:
                    append_detail(
                        details, spl=spl, contra_id=contra_id, annotation=annotation,
                        query_text=pred_row.get("query_text", ""), gold_ids=gold_ids, pred_ids=pred_ids,
                        selection_method="matched_ignored_gold", assignment_method=args.assignment,
                        semantic_score=sem_score, jaccard_score=jac_score, combined_score=combined_score,
                        decoupled=decoupled, downstream_counted=0, item_index=str(pred_row.get("item_index", "")),
                    )
                    continue
                ...
                if rel_df_indexed is not None:
                    matched_concept_tp, matched_concept_fp, matched_concept_fn = compute_tiered_concept_metrics(
                ...
                append_detail(
                    details, spl=spl, contra_id=contra_id, annotation=annotation,
                    query_text=pred_row.get("query_text", ""), gold_ids=gold_ids, pred_ids=pred_ids,
                    selection_method=f"matched_{args.assignment}", assignment_method=args.assignment,
                    semantic_score=sem_score, jaccard_score=jac_score, combined_score=combined_score,
                    extraction_tp=1, contraindication_tp=matched_contra_tp, contraindication_fp=matched_contra_fp,
                    contraindication_fn=matched_contra_fn, concept_tp=matched_concept_tp,
                    concept_fp=matched_concept_fp, concept_fn=matched_concept_fn,
                    decoupled=decoupled, downstream_counted=1, item_index=str(pred_row.get("item_index", "")),
                )
            else:
                if ignored_gold:
                    append_detail(
                        details, spl=spl, contra_id=contra_id, annotation=annotation, gold_ids=gold_ids,
                        selection_method="ignored_gold_unmatched", assignment_method=args.assignment,
                        decoupled=decoupled, downstream_counted=0,
                    )
                    continue
                ...
                append_detail(
                    details, spl=spl, contra_id=contra_id, annotation=annotation, gold_ids=gold_ids,
                    selection_method="unmatched_gold", assignment_method=args.assignment,
                    extraction_fn=1, contraindication_fn=unmatched_contra_fn, concept_fn=unmatched_concept_fn,
                    decoupled=decoupled, downstream_counted=int(not decoupled),
                )
        ...
        for pred_idx, pred_row in enumerate(pred_rows_for_matching):
            ...
            append_detail(
                details, spl=spl, query_text=pred_row.get("query_text", ""), pred_ids=pred_ids,
                selection_method="unmatched_pred", assignment_method=args.assignment,
                extraction_fp=1, contraindication_fp=unmatched_contra_fp, concept_fp=unmatched_concept_fp,
                decoupled=decoupled, downstream_counted=int(not decoupled),
    ...
    for spl in sorted(pred_only_spls):
        for pred_row in pred_by_spl[spl]:
            ...
            append_detail(
                details, spl=spl, query_text=pred_row.get("query_text", ""), pred_ids=pred_ids,
                selection_method="unmatched_pred_no_gold", assignment_method=args.assignment,
                extraction_fp=1, contraindication_fp=unmatched_contra_fp, concept_fp=unmatched_concept_fp,
                decoupled=decoupled, downstream_counted=int(not decoupled),
                item_index=str(pred_row.get("item_index", "")),
            )
    ...
    out_payload = {
        ...
        "contraindication_level": compute_metrics(contraindication_tp, contraindication_fp, contraindication_fn),
    ...
    detail_fields = [
        "SPL_SET_ID", "contra_id", "annotation", "query_text", "gold_concept_ids", "pred_concept_ids",
        "selection_method", "assignment_method", "semantic_score", "jaccard", "combined_score",
        "selected_pred_item_index", "decoupled", "downstream_counted",
        "extraction_tp", "extraction_fp", "extraction_fn",
        "contraindication_tp", "contraindication_fp", "contraindication_fn",
        "concept_tp", "concept_fp", "concept_fn",
    ]
    write_csv_file(args.out_details_csv, details, detail_fields)
